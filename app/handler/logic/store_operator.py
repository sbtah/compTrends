...
class LocalStoreOperator(BaseOperator):
    ...
    def __init__(self, local_store_id: int, *args, **kwargs):
        ...
        if not isinstance(local_store_id, int):
            raise ValueError(
                "local_store_id must be an integer representing LocalStore scraped ID."
            )
    ...
    def format_string_to_date(self, date_str: str):
        ...
        try:
            ...
            self.logger.error(
                "Error while formating string to date. String does not match desired format."
            )
            return None
    ...
    def add_missing_product_rows_to_rapport(
        ...
        if missing_ids:
            ...
            self.logger.error("No extra active IDs were found. Quiting.")
    ...
    @calculate_time
    def update_products_rapport_with_current_daily_stock_and_price(
        self,
    ) -> Union[bool, None]:
        """
        Update Products rapport for current LocalStore,
        with daily values for stock and product's prices.
        """
        try:
            df = self.open_current_xlsx_rapport()
            self.add_stock_and_price_columns_to_rapport(
                data_frame=df,
                store_id=self.local_store_id,
                date=self.current_day_month_year,
            )
            return True
        except Exception as e:
            self.logger.error(
                f"(update_products_rapport_with_current_daily_stock_and_price) Some other Exception: {e}"
            )
            return None

    def update_products_rapport_with_stock_and_price_for_dates_range(
        self, date_start_str: str, date_end_str: str
    ) -> Union[bool, None]:
        """
        Update Products rapport for current LocalStore,
        with values for stock and product's prices for given range of dates.
        """
        date_start_obj = self.format_string_to_date(date_str=date_start_str)
        date_end_obj = self.format_string_to_date(date_str=date_end_str)

        if date_start_obj and date_end_obj:
            ...
            try:
                ...
                self.logger.error(
                    f"(update_products_rapport_with_stock_and_price_for_dates_range) Some other Exception: {e}"
                )
                return None
        else:
            self.logger.error("Wrong dates provided. Quiting.")
            return None
